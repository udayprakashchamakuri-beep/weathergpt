...
app_port: 8000
pinned: false
short_description: Conversational weather intelligence for India (SIH26068)
---

<!--
  The YAML block above is Hugging Face Spaces configuration, not project docs.
  `app_port: 8000` is what routes public traffic to the container. HF Spaces
  does NOT inject $PORT -- it forwards to the port named here -- so this value
  and the PORT default in the Dockerfile must stay equal. Changing one alone
  yields a Space that builds green and serves nothing.
-->
...
The Space is a git repo; pushing to it triggers a rebuild.
...
Configuration lives in the Space's **Settings -> Variables and secrets**, never
in this repo:

| Key | Kind | Value |
|---|---|---|
| `DEMO_TOKEN` | secret | `python -c "import secrets;print(secrets.token_urlsafe(24))"` |
| `CORS_ALLOW_ORIGINS` | variable | `https://<user>-weathergpt.hf.space` |
| `ENABLE_DEMO_ENDPOINTS` | variable | `true` — `/simulate` is needed live on stage |

Two platform facts that the Dockerfile depends on:

- **HF Spaces does not inject `$PORT`.** It routes to the port named by
  `app_port:` in this README's YAML block. That is pinned to `8000` and the
  Dockerfile defaults `PORT` to the same. Change one without the other and the
  Space builds green and serves nothing.
- **The container runs as UID 1000**, so the Dockerfile creates that user
  before any `COPY` and uses `--chown=user`.

Free CPU-Basic Spaces sleep when unused; `.github/workflows/keepalive.yml`
pings `/api/health` every 10 minutes to keep the container and its cache warm.
Set the `SPACE_URL` repository variable for it to do anything.
